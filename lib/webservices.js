...
module.exports = function() {
	...
	this.getBuildStatus = function() {
		...
		console.log('set build status services');
	...
	this.getProjectInfo = function(projectId) {

		request.get({ url: self.apiBaseUrl + '/Projects/Info?projectId=' + projectId }, function callback(err, httpResponse, body) {

			if (!err) {
				self.sockets.projects.setInfo(body);
				// console.log('Get project info service call successful! Server responded with: ', body);
			} else {
				return console.error('Get project info service call failed: ', err);
			}
	...
	this.getEnvironments = function(projectId, releaseId) {
	...
	this.getLatestDeployTasks = function(projectId) {

		request.get({ url: self.apiBaseUrl + '/Deploy/GetLatestDeployTasks/?projectId=' + projectId }, function callback(err, httpResponse, body) {

			if (!err) {
				self.sockets.deploy.setLatestDeployTasks(body);
				// console.log('Get latest deploy tasks service call successful! Server responded with: ', body);
			} else {
				return console.error('Get latest deploy tasks service call failed: ', err);
			}
