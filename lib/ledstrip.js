  ...
  animate: function(redVal, greenVal, blueVal, animationTick, duration) {
    ...
    animationInterval = setInterval(function() {
      ...
      for (var i = 0; i < myDisplayBuffer.length; i += 3) {
        //red
        myDisplayBuffer[i] = redVal + Math.sin(angle + (i / 3) * ledDistance) * redVal;
        //green
        myDisplayBuffer[i + 1] = greenVal + Math.sin(angle * -5 + (i / 3) * ledDistance) * greenVal;
        //blue
        myDisplayBuffer[i + 2] = blueVal + Math.sin(angle * 7 + (i / 3) * ledDistance) * blueVal;
      }
