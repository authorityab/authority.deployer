...
module.exports = {
  ...
  /*
   * fill whole strip with one hex color
   */
  fillHex: function(hexColor) {
    var rgb = this.hexToRgb(hexColor);
    this.fillRgb(rgb.r, rgb.g, rgb.b);
  },
  ...
  /*
   * toggles between an array of hex colors
   */
  toggle: function(hexArray, speed) {
    ...
    animationInterval = setInterval(function() {
      ...
      self.fillRgb(colors[arrPos].r, colors[arrPos].g, colors[arrPos].b);
      // colors.reverse();
      arrPos++;
  ...
  /*
   * sin wave pulse of an array of colors
   */
  pulseShift: function(hexArray, speed) {
    var self = this;
    var orgArray = hexArray.slice();
    var rgb = this.hexToRgb(hexArray.shift());
    var alpha = 0;
    var angle = 0;
    var shift = false;
      ...
      if (shift) {
        if (hexArray.length > 0) {
          var hex = hexArray.shift();
	        console.log('hex: ' + hex);
          console.log('hexlength: ' + hexArray.length);
          rgb = self.hexToRgb(hex);
        } else {
          hexArray = orgArray.slice();
          rgb = self.hexToRgb(hexArray.shift());
        }
        shift = false;
      }
